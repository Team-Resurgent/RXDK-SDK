...
//////////////////////////////////////////////////////////////////////////////////
// __updateAdjForSve - update fixed_adj and sve_adj according to whether the function
//                     frame has scalable area allocated on stack
//
#if defined(_M_ARM64) && !defined(_M_ARM64EC)
extern "C" void __cdecl __updateAdjForSve(FuncInfo *pFuncInfo, PCONTEXT pCtx, int &fixed_adj, int &sve_adj)
{
    if (pFuncInfo->EHFlags & FI_FRAME_HAS_SVE)
    {
        _ASSERTE(arm64_xstate_features & XSTATE_MASK_ARM64_SVE);
        _ASSERTE((pCtx->ContextFlags & CONTEXT_ARM64_XSTATE) == CONTEXT_ARM64_XSTATE);
        _ASSERTE(locateXStateFeature);

        DWORD Length;
        auto SveBuffer = locateXStateFeature(pCtx, XSTATE_ARM64_SVE, &Length);
        _ASSERTE(SveBuffer);

        auto SveHeader = reinterpret_cast<XSAVE_ARM64_SVE_HEADER *>(SveBuffer);
        auto SveVectorLength = SveHeader->VectorLength;

        sve_adj = SveVectorLength * FUNC_DISPUNWINDHELPSVE(*pFuncInfo);
        // For 'fixed_adj', compiler generates the signed final offset directly.
        // For 'sve_adj', compiler generates number of SVE slots only, vcruntime
        // is responsible for calculating the final offset and we need negation
        // here as stack is going downwards.
        sve_adj = -sve_adj;
        fixed_adj = FUNC_DISPUNWINDHELPFIXED(*pFuncInfo);
    }
