...
private:
	...
	DName getModuleOwner(const DName &);
...
DName UnDecorator::getScope(void)
{
	...
	{	//	Insert the scope operator if not the first scope
		...
		// This name may have module ownership information.
		scope = getModuleOwner(scope);
...
DName UnDecorator::getEnumEncoding()
{
	if (*gName == '\0')
		return DName(this, DN_truncated);
	auto encoding = getTemplateTypeArgument();
...
DName UnDecorator::getScopedName(void)
{
	...
	// This name may have module ownership information.
	scopeName = getModuleOwner(scopeName);
...
DName UnDecorator::getPrimaryDataType(const DName & superType)
{
	...
	case PDT_extend:
	{
		...
		case PDT_extend_extend:
		{
			increment_buffer_no_check();
			switch (*gName)
			{
			case PDT_ex_module_owner:
				{
					increment_buffer_no_check();
					auto module_name = getZName(true);
					// Skip the '@' terminator.
					if (gName[0] == '@')
					{
						increment_buffer_no_check();
					}
					return superType + "["_l + module_name + ']';
				}
				break;
			case PDT_ex_module_owner_not_exported:
				{
					increment_buffer_no_check();
					auto module_name = getZName(true);
					// Skip the '@' terminator.
			...
			case PDT_ex_sve:
			{
				...
				case BDT_svint8:
					...
					break;
				case BDT_svint16:
					type_name = "svint16"_l;
					break;
				case BDT_svint32:
					type_name = "svint32"_l;
					break;
				case BDT_svint64:
					type_name = "svint64"_l;
					break;
				case BDT_svuint8:
					type_name = "svuint8"_l;
					break;
				case BDT_svuint16:
					type_name = "svuint16"_l;
					break;
				case BDT_svuint32:
					type_name = "svuint32"_l;
					break;
				case BDT_svuint64:
					type_name = "svuint64"_l;
					break;
				case BDT_svbfloat16:
					type_name = "svbfloat16"_l;
					break;
				case BDT_svfloat16:
					type_name = "svfloat16"_l;
					break;
				case BDT_svfloat32:
					type_name = "svfloat32"_l;
					break;
				case BDT_svfloat64:
					type_name = "svfloat64"_l;
					break;
				default:
					return DName(this, DN_invalid);
				}
				...
				{
					return type_name + "x2"_l;
				}
				else if (c == '3')
				{
					return type_name + "x3"_l;
				}
				else
				{
					return type_name + "x4"_l;
				}
			}
			default:
				return DName(this, DN_invalid);
...
DName UnDecorator::getModuleOwner(const DName& owned_name)
{
	if (gName[0] == PDT_extend
		&& gName[1] == PDT_extend
		&& gName[2] == PDT_extend_extend)
	{
		auto get_name = [&]
		{
			increment_buffer_no_check(4);
			// Enable/disable the zname behavior of rejecting names with extra characters.
			// Module names can have '.' in them, and we do not want to reject this.
			unsigned long old_flags = disableFlags;
			disableFlags |= UNDNAME_NO_IDENT_CHAR_CHECK;
			// This is a ZName.
			DName module_owner = getZName(true);
			disableFlags = old_flags;
			// Skip the '@' terminator.
			if (gName[0] == '@')
			{
				increment_buffer_no_check();
		...
		if (gName[3] == PDT_ex_module_owner)
		{
			DName module_owner = get_name();
			return owned_name + "["_l + module_owner + ']';
		}
		else if (gName[3] == PDT_ex_module_owner_not_exported)
		{
			DName module_owner = get_name();
			// Prefix with a '!' to indicate that the name is not exported.
			return owned_name + "[!"_l + module_owner + ']';
		}
	}
