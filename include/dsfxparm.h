...
/*
 * DirectSound DSP effect parameter blocks. Defines the runtime state and tuning
 * structures for each stock APU effect (amp-mod, chorus, echo, flange, IIR,
 * oscillator, I3DL2/mini reverb, RMS meter, splitter, sample-rate converter),
 * plus the FX_PARAM_DESCRIPTION metadata the effects compiler (xgpimage) emits.
 * These are read/written through IDirectSound::GetEffectData / SetEffectData at
 * the offsets named in the compiled effect image.
 */
...
//
// Metadata describing one effect's tunable parameters. The header generated by
// xgpimage carries an FX_STATE_PARAMETERS per effect, listing each parameter's
// name, byte offset into the effect state, type (INTEGER/FLOAT), default, and
// valid range -- enough for a tool to drive SetEffectData generically.
//
...
//
// Per-effect parameter blocks follow. Each _STATE struct holds the fields common
// to every effect -- scratch and Y-memory offsets/lengths (in DWORDs) within the
// downloaded effect image, dwFlags, and the input/output mixbin pointer arrays
// that wire the effect into the mixer graph. Each _PARAMS struct embeds that
// State plus the effect's own tuning coefficients.
//
