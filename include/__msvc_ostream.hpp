...
#if defined(_XBOX) // xdk_ostream_put_char
        if constexpr (is_same_v<_Elem, char>) {
            return xdk_ostream_put_char(*this, _Ch);
        }
#endif
        ios_base::iostate _State = ios_base::goodbit;
        const sentry _Ok(*this);
...
#if defined(_XBOX) // xdk_ostream_flush
        return xdk_ostream_flush(*this);
#else
...
#if defined(_XBOX)
#include "xdk_ostream_xbox_trace.h"
#endif
...
#if defined(_XBOX) // xdk_ostream_insert_cstr
    return xdk_ostream_insert_cstr(_Ostr, _Val);
#else
...
#if defined(_XBOX) // xdk_ostream_endl
    if constexpr (is_same_v<_Elem, char>) {
        xdk_ostream_put_char(_Ostr, '\n');
    } else {
        _Ostr.put(_Ostr.widen('\n'));
    }
    xdk_ostream_flush(_Ostr);
#else
    _Ostr.put(_Ostr.widen('\n'));
    _Ostr.flush();
#endif
