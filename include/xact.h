...
STDAPI_(void)
XACTEngineDoWork(void);
STDAPI_(void)
XACTEngineSetFileIOCallbacks(PCXACT_FILEIO_CALLBACKS pFileIOCallbacks);

STDAPI_(ULONG)
IXACTEngine_AddRef(PXACTENGINE pEngine);
STDAPI_(ULONG)
IXACTEngine_Release(PXACTENGINE pEngine);
...
struct IXACTEngine {
    ...
    DownloadEffectsImage(PVOID pvData, DWORD dwSize, LPCDSEFFECTIMAGELOC pEffectLoc, LPDSEFFECTIMAGEDESC* ppImageDesc)
    {
        return IXACTEngine_DownloadEffectsImage(this, pvData, dwSize, pEffectLoc, ppImageDesc);
    }

    __inline HRESULT STDMETHODCALLTYPE
    CreateSoundSource(DWORD dwFlags, PXACTSOUNDSOURCE* ppSoundSource)
    {
        return IXACTEngine_CreateSoundSource(this, dwFlags, ppSoundSource);
    }

    __inline HRESULT STDMETHODCALLTYPE
    CreateSoundBank(PVOID pvData, DWORD dwSize, PXACTSOUNDBANK* ppSoundBank)
    {
        return IXACTEngine_CreateSoundBank(this, pvData, dwSize, ppSoundBank);
    }

    __inline HRESULT STDMETHODCALLTYPE
    RegisterWaveBank(PVOID pvData, DWORD dwSize, PXACTWAVEBANK* ppWaveBank)
    {
        return IXACTEngine_RegisterWaveBank(this, pvData, dwSize, ppWaveBank);
    }

    __inline HRESULT STDMETHODCALLTYPE
    RegisterStreamedWaveBank(PCXACT_WAVEBANK_STREAMING_PARAMETERS pParams, PXACTWAVEBANK* ppWaveBank)
    {
        return IXACTEngine_RegisterStreamedWaveBank(this, pParams, ppWaveBank);
    }

    __inline HRESULT STDMETHODCALLTYPE
    UnRegisterWaveBank(PXACTWAVEBANK pWaveBank)
    {
        return IXACTEngine_UnRegisterWaveBank(this, pWaveBank);
    }

    __inline HRESULT STDMETHODCALLTYPE
    SetMasterVolume(WORD wCategory, LONG lVolume)
    {
    ...
    __inline HRESULT STDMETHODCALLTYPE
    SetListenerOrientation(FLOAT xFront, FLOAT yFront, FLOAT zFront, FLOAT xTop, FLOAT yTop, FLOAT zTop, DWORD dwApply)
    {
        return IXACTEngine_SetListenerOrientation(this, xFront, yFront, zFront, xTop, yTop, zTop, dwApply);
    }

    __inline HRESULT STDMETHODCALLTYPE
    SetListenerPosition(FLOAT x, FLOAT y, FLOAT z, DWORD dwApply)
    {
        return IXACTEngine_SetListenerPosition(this, x, y, z, dwApply);
    }

    __inline HRESULT STDMETHODCALLTYPE
    SetListenerVelocity(FLOAT x, FLOAT y, FLOAT z, DWORD dwApply)
    {
        return IXACTEngine_SetListenerVelocity(this, x, y, z, dwApply);
    }

    __inline HRESULT STDMETHODCALLTYPE
    GlobalPause(WORD wCategory, BOOL fPause)
    {
        return IXACTEngine_GlobalPause(this, wCategory, fPause);
    }

    __inline HRESULT STDMETHODCALLTYPE
    RegisterNotification(PCXACT_NOTIFICATION_DESCRIPTION pNotificationDesc)
    {
        return IXACTEngine_RegisterNotification(this, pNotificationDesc);
    }

    __inline HRESULT STDMETHODCALLTYPE
    UnRegisterNotification(PCXACT_NOTIFICATION_DESCRIPTION pNotificationDesc)
    {
        return IXACTEngine_UnRegisterNotification(this, pNotificationDesc);
    }

    __inline HRESULT STDMETHODCALLTYPE
    GetNotification(PCXACT_NOTIFICATION_DESCRIPTION pNotificationDesc, PXACT_NOTIFICATION pNotification)
    {
        return IXACTEngine_GetNotification(this, pNotificationDesc, pNotification);
    }

    __inline HRESULT STDMETHODCALLTYPE
    FlushNotification(PCXACT_NOTIFICATION_DESCRIPTION pNotificationDesc)
    {
        return IXACTEngine_FlushNotification(this, pNotificationDesc);
    }

    __inline HRESULT STDMETHODCALLTYPE
    SetVariable(DWORD dwVariable, WORD wValue, DWORD dwApply)
    {
        return IXACTEngine_SetVariable(this, dwVariable, wValue, dwApply);
    }

    __inline HRESULT STDMETHODCALLTYPE
    GetVariable(DWORD dwVariable, PWORD pwValue)
    {
        return IXACTEngine_GetVariable(this, dwVariable, pwValue);
    }

    __inline HRESULT STDMETHODCALLTYPE
    GetRealtimeData(XACT_REALTIME_AUDIO_DATA* pData)
    {
    ...
    __inline HRESULT STDMETHODCALLTYPE
    SetParameterControl(PCXACT_PARAMETER_CONTROL_DESC pParams)
    {
...
STDAPI_(ULONG)
IXACTSoundSource_AddRef(PXACTSOUNDSOURCE pSoundSource);
STDAPI_(ULONG)
IXACTSoundSource_Release(PXACTSOUNDSOURCE pSoundSource);
...
struct IXACTSoundSource {
    ...
    __inline HRESULT STDMETHODCALLTYPE
    GetStatus(PDWORD pdwStatus)
    {
        return IXACTSoundSource_GetStatus(this, pdwStatus);
    }

    __inline HRESULT STDMETHODCALLTYPE
    GetProperties(PXACT_SOUNDSOURCE_PROPERTIES pProperties)
    {
...
STDAPI_(ULONG)
IXACTSoundBank_AddRef(PXACTSOUNDBANK pBank);
STDAPI_(ULONG)
IXACTSoundBank_Release(PXACTSOUNDBANK pBank);
...
__inline HRESULT
IXACTSoundBank_Prepare(PXACTSOUNDBANK pBank, DWORD dwCueIndex, DWORD dwFlags, PXACTSOUNDCUE* ppSoundCue)
{
...
__inline HRESULT
IXACTSoundBank_Play(PXACTSOUNDBANK pBank, DWORD dwCueIndex, PXACTSOUNDSOURCE pSoundSource, DWORD dwFlags, PXACTSOUNDCUE* ppSoundCue)
{
    XACT_PREPARE_SOUNDCUE PrepareData;
...
struct IXACTSoundBank {
    ...
    __inline HRESULT STDMETHODCALLTYPE
    GetSoundCueIndexFromFriendlyName(PCSTR pFriendlyName, PDWORD pdwSoundCueIndex)
    {
        return IXACTSoundBank_GetSoundCueIndexFromFriendlyName(this, pFriendlyName, pdwSoundCueIndex);
    }

    __inline HRESULT STDMETHODCALLTYPE
    Prepare(DWORD dwSoundCueIndex, DWORD dwFlags, PXACTSOUNDCUE* ppSoundCue)
    {
        return IXACTSoundBank_Prepare(this, dwSoundCueIndex, dwFlags, ppSoundCue);
    }

    __inline HRESULT STDMETHODCALLTYPE
    PrepareEx(PCXACT_PREPARE_SOUNDCUE pPrepareData, PXACTSOUNDCUE* ppSoundCue)
    {
        return IXACTSoundBank_PrepareEx(this, pPrepareData, ppSoundCue);
    }

    __inline HRESULT STDMETHODCALLTYPE
    Play(DWORD dwSoundCueIndex, PXACTSOUNDSOURCE pSoundSource, DWORD dwFlags, PXACTSOUNDCUE* ppSoundCue)
    {
        return IXACTSoundBank_Play(this, dwSoundCueIndex, pSoundSource, dwFlags, ppSoundCue);
    }

    __inline HRESULT STDMETHODCALLTYPE
    PlayEx(PCXACT_PREPARE_SOUNDCUE pPrepareData, PXACTSOUNDCUE* ppSoundCue)
    {
        return IXACTSoundBank_PlayEx(this, pPrepareData, ppSoundCue);
    }

    __inline HRESULT STDMETHODCALLTYPE
    Stop(DWORD dwSoundCueIndex, DWORD dwFlags, PXACTSOUNDCUE pSoundCue)
    {
        return IXACTSoundBank_Stop(this, dwSoundCueIndex, dwFlags, pSoundCue);
    }

    __inline HRESULT STDMETHODCALLTYPE
    CreateWmaPlayList(DWORD dwSoundCueIndex, DWORD dwPlaybackFlags, PXACTWMAPLAYLIST* ppWmaPlayList)
    {
        return IXACTSoundBank_CreateWmaPlayList(this, dwSoundCueIndex, dwPlaybackFlags, ppWmaPlayList);
    }

    __inline HRESULT STDMETHODCALLTYPE
    SelectVariation(DWORD dwSoundCueIndex, PCXACT_SOUNDBANK_SELECT_VARIATION pVariation)
    {
        return IXACTSoundBank_SelectVariation(this, dwSoundCueIndex, pVariation);
    }

    __inline HRESULT STDMETHODCALLTYPE
    GetSoundCueProperties(DWORD dwSoundCueIndex, PXACT_SOUNDCUE_PROPERTIES pSoundCueProperties)
    {
        return IXACTSoundBank_GetSoundCueProperties(this, dwSoundCueIndex, pSoundCueProperties);
    }

    __inline HRESULT STDMETHODCALLTYPE
    PauseSoundCue(PXACTSOUNDCUE pSoundCue, BOOL fPause)
    {
...
STDAPI_(ULONG)
IXACTWmaPlayList_AddRef(PXACTWMAPLAYLIST pPlayList);
STDAPI_(ULONG)
IXACTWmaPlayList_Release(PXACTWMAPLAYLIST pPlayList);
...
struct IXACTWmaPlayList {
    ...
    __inline HRESULT STDMETHODCALLTYPE
    Add(PCXACT_WMA_PLAYLIST_ADD pDesc, PXACTWMASONG* ppSong)
    {
        return IXACTWmaPlayList_Add(this, pDesc, ppSong);
    }
    __inline HRESULT STDMETHODCALLTYPE
    Remove(PXACTWMASONG pSong)
    {
        return IXACTWmaPlayList_Remove(this, pSong);
    }

    __inline HRESULT STDMETHODCALLTYPE
    SetCurrent(PXACTWMASONG pSong)
    {
    ...
    __inline HRESULT STDMETHODCALLTYPE
    GetCurrentSongInfo(PDWORD pdwSongLength, PWCHAR pszNameBuffer, DWORD dwBufferSize, PXACTWMASONG* ppSong)
    {
        return IXACTWmaPlayList_GetCurrentSongInfo(this, pdwSongLength, pszNameBuffer, dwBufferSize, ppSong);
    }

    __inline HRESULT STDMETHODCALLTYPE
    GetCurrentSongInfoEx(PXACT_WMASONG_DESCRIPTION pDesc, PXACTWMASONG* ppSong)
    {
        return IXACTWmaPlayList_GetCurrentSongInfoEx(this, pDesc, ppSong);
    }

    __inline HRESULT STDMETHODCALLTYPE
    SetPlaybackBehavior(DWORD dwFlags)
    {
        return IXACTWmaPlayList_SetPlaybackBehavior(this, dwFlags);
    }

    __inline HRESULT STDMETHODCALLTYPE
    GetProperties(PXACT_WMA_PLAYLIST_PROPERTIES pProperties)
    {
