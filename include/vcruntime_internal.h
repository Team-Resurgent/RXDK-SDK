...
#if defined _M_ARM64
    // A few global variables initialized during startup and used later
    typedef PVOID (WINAPI *LocateXStateFeatureFn)(PCONTEXT Context, DWORD FeatureId, PDWORD Length);
    extern LocateXStateFeatureFn locateXStateFeature;
    extern DWORD64 arm64_xstate_features;

    // SVE related macros and types are not available on all SDK versions.
    // Manually define all those used in this file if XSTATE_MASK_ARM64_SVE is not defined.
    #if !defined(XSTATE_MASK_ARM64_SVE)
...
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    extern int __arm64_safe_unaligned_memory_access;
...
typedef struct RENAME_BASE_PTD(__vcrt_ptd)
{
    ...
    #if defined _M_X64 || defined _M_ARM64 || defined _M_HYBRID
    void*              _pExitContext;
...
void __cdecl __vcrt_freeptd_for_this_thread(void);
void WINAPI __vcrt_freefls(_Pre_maybenull_ _Post_invalid_ void* _Pfd);
...
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    #define __vcrt_FlsAlloc(unreferenced_callback)  TlsAlloc()
    #define __vcrt_FlsFree(fls_index)               TlsFree(fls_index)
    #define __vcrt_FlsGetValue(fls_index)           TlsGetValue(fls_index)
    #define __vcrt_FlsSetValue(fls_index, fls_data) TlsSetValue(fls_index, fls_data)
    #define __vcrt_EncodePointer(ptr)               (ptr)

#else // ^^^ Enclave ^^^ // vvv non-Enclave vvv //

    // We can directly call these APIs.
    #define __vcrt_FlsAlloc(callback)               FlsAlloc(callback)
    #define __vcrt_FlsFree(fls_index)               FlsFree(fls_index)
    #define __vcrt_FlsGetValue(fls_index)           FlsGetValue(fls_index)
    #define __vcrt_FlsSetValue(fls_index, fls_data) FlsSetValue(fls_index, fls_data)
    #define __vcrt_EncodePointer(ptr)               EncodePointer(ptr)
...
// This functionality is not supported for the enclave vcruntime.
#if defined(_CRT_WINDOWS) || defined(_VCRT_ENCLAVE_BUILD)
...
#else // ^^^ _CRT_WINDOWS || _VCRT_ENCLAVE_BUILD ^^^ // vvv Public Runtimes (except Enclave) vvv //
...
#endif // ^^^ Public Runtimes (except Enclave) ^^^ //
