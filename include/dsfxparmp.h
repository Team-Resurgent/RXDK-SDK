...
/*
 * Private/extended DirectSound DSP effect parameter blocks -- the internal
 * companion to <dsfxparm.h>, sharing its include guard so only one is active.
 * Defines the runtime state and tuning structures for each stock APU effect
 * plus the FX_PARAM_DESCRIPTION metadata the effects compiler (xgpimage) emits.
 * Read/written through IDirectSound::GetEffectData / SetEffectData.
 */
